$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(".cat-conteiner").append('<div   class="content"> <h4  class="text-black"   style="text-align: center ;">' + Noticias['TITULO'] + ' </h4><h5 class="text-black">' + Noticias['DESCRIPCION'] + ' </h5>  <strong>' + Noticias['FIRMA'] + ' </strong>  <strong style="font-style: italic;">' + Noticias['FECHA_PUBLICACION'] + ' </strong>  <strong class="datos">' + Noticias['STATUS'] + ' </strong>   <p class="datos">' + Noticias['TEXTO'] + ' </p>  <form  action="includes/subir_noticia_inc.php" class="content" method="POST"> <input type="hidden" name="ID" value="'+Noticias['NOTICIA_ID']+'"></input>    <input type="submit"id="btn_update" value="Subir noticia">   </input></form> <form  action="includes/Comentario_editor_inc.php" class="content" method="POST"> <input type="hidden" name="ID" value="'+Noticias['NOTICIA_ID']+'"></input>  <input type="text" placeholder="Comentario" name="comentario" required> <input type="submit"id="btn_update" value="Hacer comentario"></input> </form> <form  action="VistaNoticia.php" class="content" method="POST"> <input type="hidden" name="ID" value="'+Noticias['NOTICIA_ID']+'"></input>   <input type="submit"  name="submit" id="btn_update"  value="Ver noticia"></input>    </form> </div>')
